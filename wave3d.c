...
float wave_frequencies[200] = {0.3,0.1,0.9};
...
int main(int argc, char** argv){
    ...
    while(1){
		...
		for(int x=0;x<grid_width;x+=grid_step_size){//y
			for(int y=0;y<grid_width;y+=grid_step_size){//x
				for(int z=slice_z;z<slice_z+slice_width;z++){//x
					float amplitude = 0;
			    	...
			    	if(amplitude > shading_threshold){
			    		if(z < slice_z+slice_width-10){
			    			if(amplitude < shading_threshold-10){
					    		glPushMatrix();
